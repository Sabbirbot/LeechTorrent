...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text(
        f"<a href='tg://user?id={g_id}'>🔊</a> Now Uploading to ☁️ Drive!!!"
    )
    ...
    if os.path.isfile(file_upload):
        ...
        button = []
        button.append(
            [pyrogram.InlineKeyboardButton(text="☁️ Drive Link ☁️", url=f"{gauti}")]
        )
        if INDEX_LINK:
            ...
            button.append(
                [
                    pyrogram.InlineKeyboardButton(
                        text="ℹ️ Index Link ℹ️", url=f"{tam_link}"
                    )
                ]
            )
        ...
        button = []
        button.append(
            [pyrogram.InlineKeyboardButton(text="☁️ Drive Link ☁️", url=f"{gautii}")]
        )
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{os.path.basename(file_upload)}/"
            tam_link = requests.utils.requote_uri(indexurl)
